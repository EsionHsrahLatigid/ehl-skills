...
6. Treat the public identity as `EsionHsrahLatigid`; never expose the private descriptor or internal brand rationale in public copy, metadata, UI, repository text, release notes, repository metadata, public source, website/catalog text, artifacts, messages, or commit history. Keep forbidden plaintext out of repository history and use the bundled hash-based public-text guard instead of adding plaintext fixtures or prose.
